...
public class EnwikiContentSource extends ContentSource {
  ...
  public ContentDoc getNextDocData(ContentDoc docData) throws EOFException, IOException {
    ...
    docData.clear();
    docData.setID(Long.parseLong(tuple[ID]));
    docData.setBody(tuple[BODY]);
    try {
		docData.setDate(_formatter.get().parse(tuple[DATE]));
  ...
  public static void main(String[] args) throws Exception{
	...
	try{
		ContentDoc docData = new ContentDoc();
		while(true){
		  docData = wikiSrc.getNextDocData(docData);
